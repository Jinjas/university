...
class Balde():
    ...
    def expande(self,estado):
        possiveis = []
        e1_x = int(str(estado)[1])
        e1_y = int(str(estado)[3])
        if e1_x > 0:
            possiveis.append(self.esvazia1(estado))
        if e1_y > 0:
            possiveis.append(self.esvazia2(estado))
        if e1_x < self.balde1:
            possiveis.append(self.enche1(estado))
        if e1_y < self.balde2:
            possiveis.append(self.enche2(estado))
        if e1_x > 0 and e1_y < self.balde2:
            possiveis.append(self.despeja12(estado))
        if e1_y > 0 and e1_x < self.balde1:
            possiveis.append(self.despeja21(estado))
        return possiveis
    ...
    def despeja12(self, nodo):
        ...
        if ex + ey <= self.balde2:
            ey = ex + ey
            ex = 0
        else:
            falta2 = self.balde2 - ey
            ex = ex - falta2
            ey = self.balde2
        res = "(" + str(ex) + "," + str(ey) + ")"
        return res
    ...
    def despeja21(self, nodo):
        ...
        if ex + ey <= self.balde1:
            ex = ey + ex
            ey = 0
        else:
            falta1 = self.balde1 - ex
            ex = self.balde1
            ey = ey - falta1
        res = "(" + str(ex) + "," + str(ey) + ")"
        return res
